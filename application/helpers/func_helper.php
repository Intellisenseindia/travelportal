...
if(!function_exists('process_text_content_for_display')) {
    function process_text_content_for_display($text) {
        ...
        $text = clear_textarea_breaks($text);
        
        $manualLinksRegexPattern = '/<a\s+[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/';
        
        $text = preg_replace_callback($manualLinksRegexPattern, function($matches){
            return '<a href="'.e($matches[1]).'" rel="nofollow" target="_blank">'.e($matches[2]).'</a>';
        }, $text);

        // Make any URL's clickable
        $text = \app\services\utilities\Str::clickable($text);
        
        // Extract URLs and replace them with placeholders
        $urls = [];
        $placeholderPrefix = 'URL_PLACEHOLDER_';
        $text = preg_replace_callback('/<a\s+[^>]*href="([^"]*)"[^>]*>.*?<\/a>/', function ($matches) use (&$urls, $placeholderPrefix) {
            $placeholder = $placeholderPrefix . count($urls);
            $urls[$placeholder] = $matches[0]; // Store the whole <a> tag in the urls array
            return $placeholder;
        }, $text);
        
        // Escape the content
        $escapedContent = e($text);
        
        // Replace placeholders with original URLs
        foreach ($urls as $placeholder => $url) {
            $escapedContent = str_replace($placeholder, $url, $escapedContent);
        }  

        return "<div class='tw-whitespace-pre-wrap'>$escapedContent</div>";
    }
}
