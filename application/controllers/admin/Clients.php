    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $id = $this->clients_model->add($data);								$contactdata['firstname'] = $data['company'];				$contactdata['lastname'] = "";				$contactdata['email'] = $data['email'];				$contactdata['phonenumber'] = $data['phonenumber'];				$contactdata['is_primary'] = 1;				$contactdata['permissions'] = Array (1,2,3 );				$contactdata['invoice_emails'] = 'invoice_emails'; 				$contactdata['credit_note_emails'] = 'credit_note_emails';				$contactdata['contract_emails'] = 'contract_emails';								$this->clients_model->add_contact($contactdata, $id);
                if (staff_cant('view', 'customers')) {
