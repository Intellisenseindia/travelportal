...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            'company',
            db_prefix() . 'clients.company as fullname',
            db_prefix() . 'clients.email as email',
            db_prefix() . 'clients.phonenumber as phonenumber',
       //     db_prefix() . 'clients.active',		//change by sourav
        //    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'customer_groups JOIN ' . db_prefix() . 'customers_groups ON ' . db_prefix() . 'customer_groups.groupid = ' . db_prefix() . 'customers_groups.id WHERE customer_id = ' . db_prefix() . 'clients.userid ORDER by name ASC) as customerGroups',      //change by sourav 
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            //db_prefix() . 'contacts.id as contact_id',  //change by sourav
            //'lastname',			//change by sourav
            //db_prefix() . 'clients.zip as zip',		//change by sourav
        ...
        foreach ($rResult as $aRow) {
            ...
            // Primary contact
          //  $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . e(trim($aRow['fullname'])) . '</a>' : '');   // comment by sourav
